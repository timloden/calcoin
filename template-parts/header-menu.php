...
<ul id="nav_list" class="top-level-nav nav-menu">
	...
	foreach( $menuitems as $item ):
		...
		if ($icon === "") {
			$old_icon =  "ca-gov-icon-" . $item_meta['_caweb_menu_icon'][0];
			update_field('icon', $old_icon, $item->ID);
			$icon = get_field('icon', $item);
		}

		$unit = get_field('item_size', $item);
		if ($unit === "") {
			$old_unit = $item_meta['_caweb_menu_unit_size'][0];
			update_field('item_size', $old_icon, $item->ID);
			$unit = get_field('item_size', $item);
		}
